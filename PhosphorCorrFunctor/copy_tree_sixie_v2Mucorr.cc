...
int main(){
  ...
  TFile *oldfile = new TFile("/afs/cern.ch/work/s/sixie/public/EnergyScaleAndResolution/ZmumuGammaEvents/WithPhotonRegression/ZmumuGammaNtuple_DYM1050.root");
  ...
  TFile *newfile = new TFile("SIXIE_LAST_VERSION/PhotonRegression/ZmumuGammaNtuple_DYM1050_MuCorr.root","recreate");
  ...
  for (long i = 0; i < nentries; i++) {
    ...
    if(Mu1Charge == -1 && Mu2Charge == 1){
      ...
      rmcor->momcor_data(mu, -1., 0, qter);// for negative reconstructed muon
      rmcor->momcor_data(mubar, 1., 0, qter); // for positive reconstructed muon
      Mu1Pt_new = mu.Pt();
      Mu1Eta_new = mu.Eta();
      Mu1Phi_new = mu.Phi();
      Mu2Pt_new = mubar.Pt();
